...
class _ContentManagerState extends State<ContentManager> {
  ...
  Widget build(BuildContext context) {
    final filteredContent = widget.content.where((c) {
      ...
      final matchesStatus = (c.isPublished && _filterPublished) ||
          (!c.isPublished && _filterDrafts);
    ...
    final searchFiltered = widget.content
        .where(
          ...
          children: [
            ...
            if (isMobile) ...[
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    _sidebarMenuButton('Study Packs', packs.length, !_showExams,
                        () => setState(() => _showExams = false),
                        isMobile: true),
                    const SizedBox(width: 8),
                    _sidebarMenuButton('Exams', exams.length, _showExams,
              ...
              Row(
                ...
                children: [
                  Expanded(
                      flex: 1,
                      child: _buildSidebar(packs.length, exams.length,
  ...
  Future<void> _exportDeckPdf(PublicDeck deck) async {
    ...
    for (final item in raw) {
      if (item is Map) {
        questions
            .add(LocalQuizQuestion.fromJson(Map<String, dynamic>.from(item)));
      }
    }
          ...
          ? Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                        child: _bannerStatCard(
                            'MATERIALS', '${widget.content.length}',
                            isMobile: true)),
                    const SizedBox(width: 12),
                    Expanded(
                        child: _bannerStatCard(
                            'STUDENTS', '${widget.stats?.totalStudents ?? 0}',
                            isMobile: true)),
                  ],
                )
              ],
          ...
          : Row(
              ...
              children: [
                ...
                Column(
                  children: [
                    _bannerStatCard(
                        'TOTAL MATERIALS', '${widget.content.length}'),
                    const SizedBox(height: 16),
                    _bannerStatCard('ACTIVE STUDENTS',
                        '${widget.stats?.activeStudents ?? 0}'),
                  ],
                )
              ],
            ),
  ...
  Widget _buildSidebar(
      int packsCount, int examsCount, int publishedCount, int draftCount) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSearchBar(),
        ...
        Text('CONTENT ENGINE',
            style: GoogleFonts.outfit(
                ...
                letterSpacing: 1)),
        const SizedBox(height: 16),
        _sidebarMenuButton('Study Packs', packsCount, !_showExams,
            () => setState(() => _showExams = false)),
        const SizedBox(height: 8),
        _sidebarMenuButton('Exams', examsCount, _showExams,
            () => setState(() => _showExams = true)),
        const SizedBox(height: 24),
        Text('STATUS FILTER',
            style: GoogleFonts.outfit(
                ...
                letterSpacing: 1)),
        const SizedBox(height: 16),
        _buildCheckbox('Published', publishedCount, _filterPublished,
            (v) => setState(() => _filterPublished = v ?? true)),
        const SizedBox(height: 8),
        _buildCheckbox('Drafts', draftCount, _filterDrafts,
            (v) => setState(() => _filterDrafts = v ?? false)),
        const SizedBox(height: 24),
        Container(
  ...
  Widget _buildCheckbox(
      String label, int count, bool isChecked, Function(bool?) onChange) {
    return Row(
      children: [
        SizedBox(
          width: 24,
        ...
        if (displayedItems.isEmpty)
          ...
          GridView.builder(
            ...
            itemCount: displayedItems.length +
                (isMobile ? 1 : 2), // adding creation/AI cards
            itemBuilder: (context, i) {
              if (i == displayedItems.length) {
                return isMobile
                    ? _buildCreateNewCard(isMobile: true)
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      ...
                      final origin =
                          kIsWeb ? Uri.base.origin : 'https://sumquiz.xyz';
                      final url = (slug != null && slug.isNotEmpty)
                          ? '$origin/s/$slug'
                          : '$origin/deck?code=${deck.shareCode}';

                      Share.share(
                          'Check out this study material on SumQuiz: $url',
                          subject: deck.title);
                    },
                  ),
                  IconButton(
              ...
              Expanded(
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                      ...
                      foregroundColor: const Color(0xFF352554),
